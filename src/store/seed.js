...
export function buildInitialState(now = new Date()) {
  ...
  const tarefas = [
    {
      ...
      classe: 'estudar',
      deadline: toDateISO(addDays(ws, 11)),
      esforco_estimado: 180,
      status: 'INBOX',
    },
    {
      id: 'seed_t2',
  ...
  const eventos = [
    ...
    ev(ws, 0, 13, 15, 'Estudar P1 Cálculo', 'estudar', {
      rastrear_conclusao: true,
      status: 'AGENDADO',
    }),
    // AGENDADO no passado (ontem) → deriva PENDENTE sozinho (status_efetivo).
    evAbs(addDays(now, -1), 14, 15.5, 'Estudar Física', 'estudar', {
      rastrear_conclusao: true,
      status: 'AGENDADO',
    }),
    ev(ws, 1, 16, 18, 'Relatório Física', 'trabalho', {
      rastrear_conclusao: true,
      status: 'CONCLUIDO',
    }),
...
function evAbs(dia, hIni, hFim, titulo, classe, extra = {}) {
  ...
  return {
    id: `seed_ev_abs_${toDateISO(dia)}_${hIni}`,
    titulo,
    inicio: toISO(inicio),
    fim: toISO(fim),
    classe,
